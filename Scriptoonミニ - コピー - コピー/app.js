...
function checkPromptInput() {
    if (promptInput) {
        ...
        if (editTextBtn) editTextBtn.disabled = false; // Always enabled for text editing
    }
}
...
async function generateImages(mode = 'image', promptPrefix = '') {
    ...
    const targetBtn = mode === 'manga' ? generateMangaBtn : (mode === 'edit' ? editTextBtn : generateImageBtn);
    ...
    // Get user prompt and apply optional prefix
    const userPrompt = promptInput.value.trim();

    // Define manga creation prompt suffix
...
`;
    ...
    let prompt;
    if (mode === 'manga') {
        // Manga mode: append manga prompt suffix to user prompt
        prompt = userPrompt + '\n\n' + MANGA_PROMPT_SUFFIX;
    } else if (promptPrefix) {
        // Edit mode with prefix
...
`;
        ...
        generateImages('edit', textEditPrompt);
    });
}
