...
- **v1.0.29** (最新) - 画像生成状態の永続化機能（スマホを閉じても次回起動時に自動復元）
- **v1.0.28** - Fetchヘッダーエラー対策（APIキーのASCII文字チェック追加、共有時のtitle削除漏れ修正）
- **v1.0.27** - TypeError防止対策（全てのDOM要素アクセスに存在チェックを追加）
- **v1.0.26** - プロンプトと参照画像にクリアボタンを追加（ラベル右側に配置）
- **v1.0.25** - カスタムプロンプトにコピーボタンを追加（タイトル行の右側に配置）
- **v1.0.24** - クレジットチャージリンクURLを/creditsに変更
- **v1.0.23** - 共有時のタイトルを削除（シンプルな共有に変更）
- **v1.0.22** - クレジットチャージリンクのテキストを「fal.aiにクレジットをチャージ」に改善
- **v1.0.21** - API設定にクレジットチャージへのリンクを追加
- **v1.0.20** - 共有ボタン追加（Web Share API対応）、iPhone向け「画像を長押しで保存」ヒント表示
- **v1.0.19** - カスタムプロンプトのテキストボックスの高さを2倍に拡大（入力しやすさ向上）
- **v1.0.18** - スマホでのピンチ拡大操作を無効化（PWAアプリとしての操作性向上）
- **v1.0.17** - 入力データ自動保存機能（プロンプト、参照画像、出力画像をlocalStorageに自動保持）
- **v1.0.16** - 3枚画像時のレイアウト安定化（grid-auto-rows追加、カメラアイコンmax制限削除で枚数による見た目の急変を防止）
- **v1.0.15** - min-height ロック解除（ドロップゾーン・カメラボタンの高さ制限を削除し、狭い画面での自然な縮小を実現）
- **v1.0.14** - カメラボタンを相対サイズ化（580px以下でも5列グリッド対応）
- **v1.0.13** - カメラアイコンのサイズと余白を最適化（40px→32px、padding 8px→4px）
- **v1.0.12** - ドロップゾーンと画像ライブラリの重なり修正（モバイル表示改善）
- **v1.0.11** - 350px以下の狭い画面で2列グリッドにフォールバック（レスポンシブ改善）
